...
if [ ! -f "$FILE_SSL_KEY" ]; then
    ...
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout $FILE_SSL_KEY -out $FILE_SSL_CRT -subj "/C=US/ST=some-state/L=some-city/O=some-organisation/CN=symfony-app"
fi
